...
test_that("Forest plots for SCM works properly", {
  ...
  dfCovs <- createInputForestData(
    list(
      "NCIL" = c(0, 1), "FORM" = c(0, 1), "FOOD" = c(0, 1),
  ...
  dfresSCM <- getForestDFSCM(
    dfCovs = dfCovs, functionList = list(paramFunction),
...
test_that("Forest plots for EMP works properly", {
  lsExpr <- rev(
    list(
      "SEX" = expression(SEX == 2), "SEX" = expression(SEX == 1),
...
test_that("forestPlot covers the remaining layout and reference-info branches", {
  ...
  fp_stacked_tab <- forestPlot(df_mock,
    parameters = c("CL", "V"),
    stackedPlots = TRUE
  )
