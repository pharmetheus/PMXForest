...
test_that("an explicitly supplied value is not rounded", {
  r <- refResolve(mockData, "WT",
    contRef = list(WT = 123.4567), idVar = "ID",
    nsig = 3
  )
...
test_that("categorical settings cover mode, lowest and an explicit level", {
  ...
  expect_equal(refResolve(mockData, "RACE",
    catRef = list(RACE = 3),
    idVar = "ID"
  )$RACE$value, 3)
})
...
test_that("\"model\" also accepts a createParamFunction() result", {
  d <- simData()
  gen <- suppressWarnings(createParamFunction(modFile,
    parameters = "CL",
    quiet = TRUE
  ))
  fromPath <- suppressWarnings(refResolve(d, "WT",
    contRef = "model",
    model = modFile, idVar = "ID"
  ))
...
test_that("\"model\" without a model, or for an underivable covariate, errors", {
  d <- simData()
  expect_error(
    refResolve(d, "WT", contRef = "model", idVar = "ID"),
    "`model` is required"
  )
  # CRCL is not referenced in run7.mod's $PK at all
  expect_error(
    refResolve(d, "CRCL",
      contRef = "model", model = modFile,
      idVar = "ID"
    ),
    "No reference value for covariate CRCL"
  )
  expect_error(
    refResolve(d, "WT", contRef = "model", model = 42, idVar = "ID"),
    "control stream path"
  )
})
...
test_that("a setting inappropriate to the covariate type is rejected", {
  expect_error(
    refResolve(mockData, "SEX", catRef = "mean", idVar = "ID"),
    "not a reference for the categorical"
  )
  expect_error(
    refResolve(mockData, "WT", contRef = "mode", idVar = "ID"),
    "not a reference for the continuous"
  )
  expect_error(
    refResolve(mockData, "SEX", catRef = "median", idVar = "ID"),
    "not a reference for the categorical"
  )
  expect_error(
    refResolve(mockData, "WT", contRef = "nonsense", idVar = "ID"),
    "Unknown reference setting"
  )
  expect_error(
    refResolve(mockData, "WT", contRef = c(1, 2), idVar = "ID"),
    "single value"
  )
})
...
test_that("references are computed on deduplicated data", {
  ...
  expect_equal(
    refResolve(dup, "WT", contRef = "median", idVar = "ID")$WT$value,
...
test_that("refLevels is deprecated but still honoured", {
  ...
  expect_warning(
    getCovStats(mockData, "RACE", refLevels = list(RACE = 2)),
    "deprecated"
  )
  expect_warning(
    setupCovExpressionsList(mockData, "RACE",
      minSubjects = 1,
...
test_that("supplying both catRef and refLevels is an error", {
  expect_error(
    suppressWarnings(getCovStats(mockData, "RACE",
      refLevels = list(RACE = 2),
      catRef = list(RACE = 1)
    )),
    "not both"
...
test_that("setupDfRefRow with model references removes the reference mismatch", {
  ...
  modelRef <- setupDfRefRow(dfCovs, d, covs,
    idVar = "ID", contRef = "model",
...
test_that("a model-derived reference row makes the inactive rows sit at 1", {
  ...
  covs <- c("WT", "SEX", "FOOD", "FORM")
  dfCovs <- setupDfCovs(d, covariates = covs, idVar = "ID")
  dfRef <- setupDfRefRow(dfCovs, d, covs,
    idVar = "ID", contRef = "model",
    catRef = "model", model = modFile
  )

  gen <- createParamFunction(modFile,
    parameters = c("CL", "V"),
...
test_that("catRef = \"model\" for the encoding level errors when underivable", {
  ...
  expect_error(
    getCovStats(d, "RACEL", idVar = "ID", catRef = "nonsense"),
    "Unknown reference setting"
  )
})
...
test_that("a covariate absent from the data is reported by name", {
  expect_error(
    refResolve(mockData, "NOPE", idVar = "ID"),
...
test_that("\"lowest\" is rejected for a continuous covariate", {
  expect_error(
    refResolve(mockData, "WT", contRef = "lowest", idVar = "ID"),
    "not a reference for the continuous"
  )
})
