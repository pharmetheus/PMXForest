test_that("mvrnorm_vector samples correctly", {
  ...
  dfParameters <- as.data.frame(mvrnorm_vector(
    mu = mu, sigma = sigma,
    iSampleIndex = 10,
    fixed_mu = fixedmu,
    dSeed = dSeed_val
...
test_that("mvrnorm_vector handles edge cases and default logic", {
  ...
  res_null_fixed <- mvrnorm_vector(
    mu = c(1, 1), sigma = diag(2),
