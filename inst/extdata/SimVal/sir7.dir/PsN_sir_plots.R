...
#Created 2022-04-13 at 20:39

xpose.runno <- '7'
toolname <- 'sir'
pdf.filename <- paste0('PsN_',toolname,'_plots.pdf')
working.directory<-'/share/Projects/Pharmetheus/PMX-REP-PMX-2/Analysis/Model/SimVal/sir7.dir/'
results.directory <- '..'
...
raw.results.file <- 'raw_results_run7.csv'
...
ALL.RAWRESFILES   <-c('raw_results_sir_iteration1.csv','raw_results_sir_iteration2.csv','raw_results_run7.csv')
MAX.RESAMPLE <- 1    # maximum resamples for single sample. 1 if without replacement
COL.ESTIMATED.PARAMS <-  c(32:48)   #column numbers in raw_results for parameters
N.ESTIMATED.PARAMS <- 17
N.ESTIMATED.THETAS <- 11
