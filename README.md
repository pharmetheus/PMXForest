...
Everything below uses the `run7` example bundled with the package, so it
runs anywhere `PMXForest` is installed.
...
`filterByModel()` applies the model’s own `IGNORE`/`ACCEPT` statements,
so the covariate summaries below describe the subjects the model was
actually fitted to.

``` r
dfData <- filterByModel(read.csv(dataFile), modFile)
#> Applying IGNORE: (TYPE == 2) | (BLQ == 1) | (ID == 895)
#> Removed 35131 of 69016 record(s) and 210 subject(s).
```

### Specify the covariate information

`setupDfCovs()` turns the data into one row per Forest plot row: the 5th
and 95th percentiles of a continuous covariate, one row per level of a
categorical one. `catRef = list(GENO = 2)` makes 2D6 EM the reference
and `sep = ""` names the one-hot columns the way the control stream
does.
...
The parameter function turns the estimates and one row of covariate
values into the quantities to plot. `createParamFunction()` writes it by
translating the model’s `$PK` block, so the relationship is not retyped
in R and cannot drift away from the model. `secondary` adds `AUC` after
an 80 mg dose, which is not in `$PK` because the dose is not.
...
The result is source text. Read it against the control stream, then
evaluate it:
...
PMXForest takes care of the simulations and predictions needed for the
plot, including the uncertainty. Here it is taken from the NONMEM `$COV`
step: `getSamples()` draws the parameter vectors and `getForestDFSCM()`
does the calculations, in parallel if asked. `gen$noBaseThetas` is the
THETA count read from the model, so it does not have to be typed in and
kept up to date.
...
<img src="man/figures/README-forest-plot-1.png" alt="" width="100%" />
