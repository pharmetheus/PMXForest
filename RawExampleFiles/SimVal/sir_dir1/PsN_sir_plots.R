...
#Created 2020-04-29 at 17:07

rplots.level <- 1
xpose.runno <- '1'
toolname <- 'sir'
pdf.filename <- paste0('PsN_',toolname,'_plots.pdf')
pdf.title <- 'sir diagnostic plots run 1'
working.directory<-'/home/shared/Projects/Pharmetheus/PMX-MI-PMX-1/Analysis/Model/FRX/sir_dir1/'
...
tool.results.file <- 'sir_results.csv'
...
SAMPLES          <-2000
RESAMPLES        <-1000
ALL.SAMPLES      <-c(1000,1000,2000)
ALL.RESAMPLES    <-c(200,500,1000)
ALL.RAWRESFILES   <-c('raw_results_sir_iteration1.csv','raw_results_sir_iteration2.csv','raw_results_run1.csv')
MAX.RESAMPLE <- 1    # maximum resamples for single sample. 1 if without replacement
COL.ESTIMATED.PARAMS <-  c(32:50)   #column numbers in raw_results for parameters
N.ESTIMATED.PARAMS <- 19
N.ESTIMATED.THETAS <- 13
N.ESTIMATED.OMEGAS <- 5
N.ESTIMATED.SIGMAS <- 1
model.subjects <- 755
CI <- 95
