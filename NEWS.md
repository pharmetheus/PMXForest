...
* **Shared one-hot encoder:** Added `oneHotEncode()`, which adds `<covariate><sep><level>` dummy columns to a data frame following the convention shared with NONMEM FREM data sets and `PMXFrem::createFREMModel()`. It is idempotent on data that already carries the dummy columns.
...
* **`oneHot` in the forest-data functions:** `getForestDFSCM()` and `getForestDFemp()` gained an optional `oneHot` argument. When supplied, raw multi-level categorical columns are one-hot encoded before the parameter functions run - in `dfCovs`/`dfRefRow` for `getForestDFSCM()`, in `dfData` for `getForestDFemp()` - so a single parameter function written against the dummy columns serves both workflows. Defaults to `NULL` (no encoding, output unchanged).
