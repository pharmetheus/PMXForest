...
## Bug Fixes
* **`verifyParamFunction()` on a secondary parameter:** Naming a `secondary` parameter explicitly (e.g. `parameters = "AUC"`) crashed with "subscript out of bounds" instead of warning that it could not be reconciled against the table - `x$etaMap[[p]]` throws rather than returning nothing when `p` has no entry, which every secondary parameter doesn't. Now guarded so the intended warning ("Cannot recover typical values...") and `PASS = NA` row are produced instead. Found via `covr::package_coverage()`, which exercises this path; the routine `devtools::test()` suite did not (the test helper had the same bug, masking it).
