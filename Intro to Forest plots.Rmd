...
# Preliminaries
Some of the functions in this vignette take quite some time to run and the output from these functions are therefore precomputed. To determine if the precomputed result should be used is governed by the flag `reCompute`. (TRUE means that the results will be re-computed.)
```{r}
reCompute <- TRUE
```
...
if(reCompute) {
  ...
  #save(dfresBS,file="dfresBS.RData")
} else {
  #load("dfresBS.RData")
  load(system.file("extdata","vingetteData/dfresBS.RData",package="PMXForest"))
}
```
...
if(reCompute) {
  ...
  #save(dfresSIR,file="dfresSIR.RData")
} else {
  load(system.file("extdata","vingetteData/dfresSIR.RData",package="PMXForest"))
  #load("dfresSIR.RData")
}
```
...
if(reCompute) {
  ...
  #save(dfresCOVemp,file="dfresCOVemp.RData")
} else {
  #load("dfresCOVemp.RData")
  load(system.file("extdata","vingetteData/dfresCOVemp.RData",package="PMXForest"))
}

```

Compute the data to be plotted based on a small bootstrap:
```{r BSsmallplotDataEmp}
if(reCompute) {
  dfresBSsmallemp <- getPlotDataEmp(dfSamplesBSsmall,dfRefRow = dfRefRow)
  #save(dfresBSsmallemp,file="dfresBSsmallemp.RData")
} else {
  #load("dfresBSsmallemp.RData")
   load(system.file("extdata","vingetteData/dfresBSsmallemp.RData",package="PMXForest"))
}
...
if(reCompute) {
  ...
  #save(dfresBSemp,file="dfresBSemp.RData")
} else {
  #load("dfresBSemp.RData")
  load(system.file("extdata","vingetteData/dfresBSemp.RData",package="PMXForest"))
}
```
...
if(reCompute) {
  ...
  #save(dfresSIRemp,file="dfresSIRemp.RData")
} else {
  #load("dfresSIRemp.RData")
  load(system.file("extdata","vingetteData/dfresSIRemp.RData",package="PMXForest"))
}
