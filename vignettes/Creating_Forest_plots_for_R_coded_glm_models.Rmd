...
output: 
  bookdown::html_document2:
    toc: true
vignette: >
  %\VignetteIndexEntry{Creating_Forest_plots_for_R_coded_glm_models}
...
#The distance in forest plot between groups
groups_dist<-0.2
#The distance in forest plot within groups
within_groups_dist<-0.1
...
for (i in 1:nrow(dfres)) {
  ...
  dfres$FUNC[i]<-100*q[2] #Median 
  dfres$q2[i]<-100*q[3] #Upper CI
  if (dfres$GROUP[i]!=gri) { #New group
    y<-y+groups_dist
    gri<-dfres$GROUP[i]
  } else { #Old group
    y<-y+within_groups_dist
  }  
  dfres$Y[i]<-y
}
...
```{r ForestSurvival,fig.width=10,fig.asp=.5,out.width="100%"}
p<-PMXForest::plotForestDF(dfres,plotRelative = FALSE,strxlab = "")
p+geom_vline(aes(xintercept=REFTRUE),size=1.5,linetype="dashed")
