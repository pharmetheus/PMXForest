...
Forest plots is an effective way of communicating the impact of covariates. The principal is that the predicted effect of a particular value of the covariate on a primary or secondary parameter is visualized together with a measure of the uncertainty around the predicted parameter value. By calculating and visualising the impact of different covariates and covariate values, inference can be made about the relative importance of the covariates. An example Forest plot, with important features highlighted, is provided in Figure \@ref(fig:exampleForest).
...
## Standardized covariate model visualisation at Pharmetheus
...
* Table woth predicted covariate effects  
...
In the following the document is organised in two parts. The first concerns parametric Forest plots and the second empirical Forest plots. The two approaches have a great deal in common and share many `PMXForest` functions. The first part, about the parameteric Forest plots, will therefore provide most of the details about the implementation while the second part will only highlight the required differences to the fir part.
...
With the funcitons i place it is quite straight forward to compute the  predicted parameter values. It is just to communicate the covariate values of interest to the functions (see Section \@ref(sec:dfCovs)).
...
The uncertainties can also be obtained non-parametrically. The bootstrap is an example of such a method, sampling importance resampling (SIR) is another. The main benefit with these approaches is the lack of symmetry assumptions - the derived confidence intervals will be assymetrical if so indicated by the data. The drawback is that these methods can take quite some time to run and that they require an extra effort after the covariate model has been established.

The `PMXForest` package has generalised the uncertainty assessment so that the the same procedure can be used regardless of the underlying method to obtain the uncertainty in the primary parameters, if it is a parametric or empirical Forest plot or if the covariate analysis strategy is FREM or scm(plus).

The uncertainties in the preducted parameter values are calculated in two steps:
...
In this method, a parametric variance-covariance matrix is computed from a limited set of bootstrap samples and the distribution of the primary parameters is obtained by multivariate sampling from this matrix. The computed uncertainty will be symmetric and will rely on normal distribution assumptions. You would use this method in case the \$COV from NONMEM is unsuccesful (or for some reason is unreliable) and the runtimes makes a full boostrap or a SIR unpractical.
3. Non-parametric uncertainty estimates from a large bootstrap (n=250).  
A full boostrap will result in uncertainties that does not rely on normality assumtions and whose confidence intervals can be non-symmetrical. The multivariate distribution is taken as the boostrap estimates and it is therefore important to have a suffiently large n. The main drawback of this approach is obvioulsy the run times but n=250 will result in a precision for the percentile limits of a 90% confidence interval that meet the Pharmetheus standards. 
4. Uncertainty estimates from a SIR analysis.  
The samples available from a SIR analysis is used as the multivariate distribution of the primary parameter estimates. This method would be used if a SIR is already available or if SIR is judged to be the preferred method of obtaining the uncertainies.
...
The `groupdist`and `withingroupdist´`arguments to the `getForestDFSCM` can be used to finetune how the covariate names are displayed in the Forest plots.
...
This approach will sample parmeter vectors from a variance-covariance matrix from NONMEM.

Read the cov file and get the mutivariate parameer distribution.
```{r}
...
Empirical Forest plots derive the predicted covariate effects based on the mutlivariate covariate distribution in the current data set. The difference to what is referred to as parametric predictions in this document is that the empirical predictions are taken as summary measures of the predicted typical individual parameter estimates from the model and observed covariates. Parametric predictions, in contrast, are derived directly from the fixed effects parameter estimates of the model and specific covariate values of interest. 
...
With the above in mind it is obvious that empirical Forest plots rely on an observed multivariate covariate distribution while parametric Fores plots can use any covariate value regardles of if it is included in the observed adta range or not. It is also clear that empirical Forest plots can visualise the impact of all available covariates regardless of if they are included in the model or not (in the above example it is not necessary that the model includes SEX, the predicted individual CL values can still be split by SEX). Parametric Forest plots would by default predict no impact of covariates not included in the model.
