...
runno <- 1
extFile <- system.file("extdata/oldExample", paste0("run", runno, ".ext"), package = "PMXForest")
...
```{r}
bootFile <- system.file("extdata/oldExample", "bs1_n100.dir", paste0("raw_results_run", runno, ".csv"), package = "PMXForest")
...
#plotList <- forestPlot(dfresmrgsolveCmax,plotRelative = TRUE,keepYlabs = T,rightStrip = F,return="plotList")
...
```{r Cmaxmrgsolve2, cache=TRUE}
# Cmax
dfresmrgsolveCmaxAUC <- getForestDFSCM(
  dfCovs = dfCovs,
  cdfCovsNames = covnames,
  functionList = list(Cmax,AUC),
  functionListName = c("CMAX","AUC"),
...
```{r ForestCOVMRGSOLVE2,fig.width=12,fig.asp=0.6,out.width="100%"}
plotList <- forestPlot(dfresmrgsolveCmaxAUC,plotRelative = TRUE,keepYlabs = T,rightStrip = F,return="plotList")
...
Please see the vignette and reference material for the `PMXForest`, `deSolve` and `mrgsolve` packages for more information.
