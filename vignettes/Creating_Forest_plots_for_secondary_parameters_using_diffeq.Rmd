---
title: "Creating Forest plots for scm models defined using differential equations"
output: 
...
In this example the focus is not the different parts of constructing a forest plot but narrowing in on the parts relevant for producing a forest plot based on secondary parameters using a non-trivial design (e.g. multiple doses) or a single dose model defined as differential equations. 

To illustrated this we will use two different packages, `deSolve` for the standard ordinary differential equations (ode) following a single dose, and `mrgsolve` which can be used if we have more advanced designs. Examples of secondary parameters derived using optimization (e.g. Cmax) is also provided.
...
# Creating a parametric Forest plots with a deSolve model
...
The `functionList` argument to `getForestDFSCM` is a list of functions that computes the parameters to plot in the Forest plot from the NONMEM parameter estimates. More than one function can be provided as is exemplified below.
...
In this first example a single dose model function is defined using the ´deSolve` differential equations package.

```{r}

#This function calculates the concentration at any given times as well as the AUC
# assuming a single dose at time 0. 
...
Cmax<-function(thetas, df, interval) {
  ...
  return(optimize(f = wrapper,interval = interval,
                  maximum = TRUE,thetas=thetas,df=df)$objective)
}
...
dfExt <- subset(PMXForest::getExt(extFile = extFile), ITERATION == "-1000000000")
...
dfSamplesBS <- PMXForest::getSamples(bootFile,extFile=extFile)
```
...
# Creating a parametric Forest plots from a mrgsolve model
...
Cmax<-function(thetas, df, interval,tau,model) {
  ...
  return(optimize(f = wrapper,interval = interval,
                  maximum = TRUE,tol = 1E-03,thetas=thetas,df=df,tau,model)$objective)
}
...
Next step is to compile the mrgsolve function in order to pass it as a variable into the parameter function (`paramFunctionmrgsolve`).
...
A good way to check the model is now to plot the parameter function output
...
#Cmax
  dfresmrgsolveCmax <- getForestDFSCM(dfCovs = dfCovs,
                          cdfCovsNames = covnames,
...
#AUC
  dfresmrgsolveAuc <- getForestDFSCM(dfCovs = dfCovs,
                          cdfCovsNames = covnames,
...
```{r ForestCOVMRGSOLVE,fig.width=12,fig.asp=0.6,out.width="100%"}
plotForestDF(rbind(dfresmrgsolveCmax,dfresmrgsolveAuc),
             log10x = TRUE,
             addTable=TRUE,
             percentscalexmax = 7,
             useRefUncertainty = FALSE)+
